...
import AutoEnc_RNN_Seq2Seq
import AutoEnc_RNN_phoneme_Seq2Seq
import AutoEnc_model_RNN_Seq2Seq
...
if model_selection == 'Seq2Seq':
    ...
    enc_phoneme = AutoEnc_RNN_phoneme_Seq2Seq.RNN_Encoder(41, ENC_EMB_DIM, ENC_HID_DIM, 'LSTM', device, N_LAYER, ENC_DROPOUT).to(device)
    dec_phoneme = AutoEnc_RNN_phoneme_Seq2Seq.RNN_Decoder_2(DEC_HID_DIM, 41, ENC_HID_DIM, attn_phoneme, DEC_DROPOUT, 'LSTM', device, N_LAYER).to(device)  
    phoneme_generator = AutoEnc_RNN_phoneme_Seq2Seq.Seq2Seq_Generator_1(enc_phoneme, dec_phoneme, device).to(device)   
    checkpoint = torch.load('./saved_checkpoints/Seq2Seq_phoneme_autoenc_LSTM2_epoch_10.pth',map_location=device)
    phoneme_generator.load_state_dict(checkpoint['model_state_dict'])
    
    attn_AU = AutoEnc_RNN_Seq2Seq.Attention(ENC_HID_DIM, DEC_HID_DIM).to(device)
    enc_AU = AutoEnc_RNN_Seq2Seq.RNN_Encoder(16, ENC_EMB_DIM, ENC_HID_DIM, 'LSTM', device, N_LAYER, ENC_DROPOUT).to(device)
    dec_AU = AutoEnc_RNN_Seq2Seq.RNN_Decoder_2(DEC_HID_DIM, 16, ENC_HID_DIM, attn_AU, DEC_DROPOUT, 'LSTM', device, N_LAYER).to(device)
    au_enc_generator = AutoEnc_RNN_Seq2Seq.Seq2Seq_Generator_1(enc_AU, dec_AU, device).to(device)         
    checkpoint = torch.load('./saved_checkpoints/Seq2Seq_AutoEnc_LSTM_2_epoch_100.pth',map_location=device)
    au_enc_generator.load_state_dict(checkpoint['model_state_dict'])
  
    
    attn = AutoEnc_model_RNN_Seq2Seq.Attention(ENC_HID_DIM, DEC_HID_DIM).to(device)   
    au_generator = AutoEnc_model_RNN_Seq2Seq.Seq2Seq_Generator_1(phoneme_generator.encoder, au_enc_generator.decoder, device).to(device) 
    
    
#    au_descriminator = AU_Discriminator(OUTPUT_DIM, DEC_HID_DIM, N_LAYER, 'LSTM').to(device)
    for param in au_generator.encoder.parameters():
        param.requires_grad = False
    
    for param in au_generator.decoder.parameters():
        param.requires_grad = False
...
#    optimizer_D = optim.Adam(au_descriminator.parameters(), lr=learning_rate)
    ...
    criterion_model = loss_func.loss_cosine_MSE_seq(beta=0)
...
#            print ('iteration %d end: %f' % (i,loss.item()))    
        ...
        encoder_grad = list()
        for p in list(filter(lambda p: p.grad is not None, model.encoding_hidden.parameters())):
    ...
    if(load_epoch):
        print('test')
        checkpoint = torch.load('./checkpoints/Seq2Seq_AutoEnc_LSTM_2_epoch_100.pth',map_location=device)
...
#            len_index=3
#            for j in range(0,10):
            train_loss, dataloader_len = train_Seq2Seq(au_generator, au_dataloader_list[len_index] , optimizer_G, criterion_model, 1, 0)
...
#            plot_loss_total_val = 0
        ...
        if epoch % save_every == 0:
            ...
            }, './checkpoints/Seq2Seq_combine_fix_encdec_LSTM_2_epoch_%d.pth' % epoch)
