...
import AutoEnc_RNN_AU_Seq2Seq
import AutoEnc_RNN_phoneme_Seq2Seq
import AutoEnc_model_hidden_RNN_Seq2Seq
...
#file_names = sorted(glob.glob("./Data_Preprocess/train_AUs_LRW_id/*.mat"))
file_names = sorted(glob.glob("/eecf/cbcsl/data100b/zheng.1443/Data/Word_Frames/train_AUs_LRW/*.mat"))
...
if model_selection == 'Seq2Seq':
    ...
    attn_phoneme = AutoEnc_RNN_phoneme_Seq2Seq.Attention(ENC_HID_DIM, DEC_HID_DIM).to(device)
    enc_phoneme = AutoEnc_RNN_phoneme_Seq2Seq.RNN_Encoder(41, ENC_EMB_DIM, ENC_HID_DIM, 'GRU', device, N_LAYER, ENC_DROPOUT).to(device)
    dec_phoneme = AutoEnc_RNN_phoneme_Seq2Seq.RNN_Decoder_2(DEC_HID_DIM, 41, ENC_HID_DIM, attn_phoneme, DEC_DROPOUT, 'GRU', device, N_LAYER).to(device)  
    phoneme_generator = AutoEnc_RNN_phoneme_Seq2Seq.Seq2Seq_Generator_1(enc_phoneme, dec_phoneme, device).to(device)   
    checkpoint = torch.load('./saved_checkpoints/Seq2Seq_phoneme_autoenc_GRU2_epoch_15.pth',map_location=device)
    phoneme_generator.load_state_dict(checkpoint['model_state_dict'])
    
    attn_AU = AutoEnc_RNN_AU_Seq2Seq.Attention(ENC_HID_DIM, DEC_HID_DIM).to(device)
    enc_AU = AutoEnc_RNN_AU_Seq2Seq.RNN_Encoder(16, ENC_EMB_DIM, ENC_HID_DIM, 'GRU', device, N_LAYER, ENC_DROPOUT).to(device)
    dec_AU = AutoEnc_RNN_AU_Seq2Seq.RNN_Decoder_2(DEC_HID_DIM, 16, ENC_HID_DIM, attn_AU, DEC_DROPOUT, 'GRU', device, N_LAYER).to(device)
    au_enc_generator = AutoEnc_RNN_AU_Seq2Seq.Seq2Seq_Generator_1(enc_AU, dec_AU, device).to(device)         
    checkpoint = torch.load('./saved_checkpoints/Seq2Seq_AU_beta0.8_AutoEnc_GRU_2_epoch_100.pth',map_location=device)
    au_enc_generator.load_state_dict(checkpoint['model_state_dict'])
  
    
#    attn = AutoEnc_model_hidden_RNN_Seq2Seq.Attention(ENC_HID_DIM, DEC_HID_DIM).to(device)   
    hidden_generator = AutoEnc_model_hidden_RNN_Seq2Seq.Seq2Seq_Generator_1(device).to(device) 
   
   
#    au_descriminator = AU_Discriminator(OUTPUT_DIM, DEC_HID_DIM, N_LAYER, 'LSTM').to(device)
#    for param in hidden_generator.encoder.parameters():
#        param.requires_grad = False
#    
#    for param in hidden_generator.decoder.parameters():
#        param.requires_grad = False

    print ('N parameters: %d' % util_func.count_parameters(hidden_generator))
    
    learning_rate=0.0001
    
    optimizer_G = optim.Adam(hidden_generator.parameters(), lr=learning_rate)
...
#    criterion_model = loss_func.loss_cosine_MSE_seq(beta=0.8)
    ...
    criterion_model = loss_func.loss_hidden_MSE(beta=0)
...
#            for j in range(1,150):
            ...
            with torch.no_grad():
                encoder_hidden = phoneme_generator.encoder.initHidden(mini_batch_size)
                encoder_input  = input_tensor.permute(1,0)
                encoder_outputs, encoder_hidden = phoneme_generator.encoder(encoder_input, encoder_hidden, input_len)
                
                decoder_hidden = au_enc_generator.encoder.initHidden(mini_batch_size)
                decoder_input = target_tensor.permute(1,0,2)
                decoder_outputs, decoder_hidden = au_enc_generator.encoder(decoder_input, decoder_hidden, input_len)
                
            
            
        
#            temp_output = au_enc_generator(target_tensor,input_len,target_tensor,0)
#            temp_output = temp_output.permute(1,0,2)
                
#            temp_output = phoneme_generator(input_tensor, input_len, input_tensor,p_id,0)
...
#            temp_output.max(1)[1]
            ...
            new_hidden = hidden_generator(encoder_hidden, p_id)
            new_hidden = new_hidden.view(mini_batch_size,2,128)
            
#            new_hidden = torch.cat((new_hidden[0],new_hidden[1]),dim=0)
#            new_hidden = new_hidden.permute(1,0,2)
#            decoder_hidden = torch.cat((decoder_hidden[0],decoder_hidden[1]),dim=0)
#            decoder_hidden = decoder_hidden.permute(1,0,2)

            decoder_hidden = decoder_hidden.permute(1,0,2)            
            loss = criterion.loss_fn(new_hidden, decoder_hidden)
#                if j==1:
...
#        decoder_grad = list()
#        for p in list(filter(lambda p: p.grad is not None, model.encoding_cell.parameters())):
...
#        print(decoder_grad)
    ...
    if(load_epoch):
        print('test')
        checkpoint = torch.load('./checkpoints/Seq2Seq_batchnorm_combine_hidden_encdec_LSTM_2_epoch_615.pth',map_location=device)
        plot_losses = checkpoint['train_loss']
        plot_losses_val = checkpoint['val_loss']
        epoch_start = checkpoint['epoch']       
        hidden_generator.load_state_dict(checkpoint['model_state_dict'])
        optimizer_G.load_state_dict(checkpoint['model_optimizer_state_dict'])

        
#        pretrained_dict = checkpoint['model_state_dict']
#        model_dict = au_generator.decoder.state_dict()
#
...
#        au_generator.decoder.load_state_dict(pretrained_dict)
...
#            for j in range(0,100):
            train_loss, dataloader_len = train_Seq2Seq(hidden_generator, au_dataloader_list[len_index] , optimizer_G, criterion_model, 1, 0)
            print('train %d: (%f)' % (len_index, train_loss/dataloader_len))
...
#            plot_loss_total_val = 0
        ...
        if epoch % save_every == 0:
            torch.save({
            'epoch': epoch,
            'model_state_dict': hidden_generator.state_dict(),
            'model_optimizer_state_dict': optimizer_G.state_dict(),
            'train_loss': plot_losses,            
            'val_loss': plot_losses_val,
            }, './checkpoints/Seq2Seq_tanh_BN_combine_hidden_encdec_LSTM_2_epoch_%d.pth' % epoch)
