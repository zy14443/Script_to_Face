...
#        output = self.out_decoder(torch.cat((output, weighted, input,cell_id), dim=1))         
        output = self.tanh(self.out(torch.cat((output, weighted, input,cell_id), dim=1)))
...
class RNN_Decoder_2(nn.Module):
    def __init__(self, dec_hid_dim, output_dim, enc_hid_dim, attention, dropout, RNN_type, device, num_layers=1):
        ...
        self.out_decoder = make_mlp([dec_hid_dim,128,128], activation='relu', batch_norm=False, dropout=0)        
...
#        
#        rnn_input = torch.cat((input, weighted), dim=2)
        rnn_input = self.input_encoder(input)
...
#        self.decoder = decoder
        self.device = device
#        self.embedding = nn.Embedding(1100, 128)
#        self.pid_embedding = make_mlp([128,256,512,1024], activation='leakyrelu', batch_norm=True, dropout=0)

#        self.hidden_embedding = make_mlp([256,512,1024], activation='leakyrelu', batch_norm=True, dropout=0)
        self.encoding_hidden = make_mlp([256+128, 256, 256, 512, 512, 1024, 1024, 512, 512, 256], activation='leakyrelu', batch_norm=True, dropout=0)
        self.encoding_hidden_out = nn.Linear(256,256)
        self.tanh = nn.Tanh()
#        self.cell_embedding = make_mlp([128,256,512], activation='relu', batch_norm=False, dropout=0)
#        self.encoding_cell = make_mlp([1024,512,256], activation='relu', batch_norm=False, dropout=0)
#        self.encoding_cell_out = nn.Linear(256,128)
    def create_mask(self, src):
        mask = (src != PAD_IDX).permute(1, 0)
        return mask 
    
    def forward(self, hidden,p_id):
...
#       
#        p_id = torch.cat((p_id,p_id),dim=2)
        p_id = p_id.squeeze()
#        p_id = self.embedding(p_id)
#        p_id = self.pid_embedding(p_id)
#        hidden = hidden.contiguous()
#        hidden = hidden.view(256,256)
        hidden = torch.cat((hidden[0,].squeeze(0), hidden[1,].squeeze(0)),dim=1)
#        hidden_emb = self.hidden_embedding(hidden)
#        cell_emb = self.cell_embedding(hidden[1])
...
#        new_hidden = (hidden[0],cell_id)
        hidden_0 = self.tanh(self.encoding_hidden_out(self.encoding_hidden(torch.cat((hidden, p_id),dim=1))))
#        hidden_1 = self.encoding_cell_out(self.encoding_cell(torch.cat((cell_emb,p_id),dim=2)))
#        new_hidden = (hidden_0,hidden_1)
