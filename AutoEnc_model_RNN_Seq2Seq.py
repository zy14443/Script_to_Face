...
class Seq2Seq_Generator_1(nn.Module):
    def __init__ (self, encoder, decoder, device):
        ...
        self.device = device
        self.encoding_hidden = make_mlp([256,256,128], activation='relu', batch_norm=False, dropout=0)
        self.encoding_hidden_out = nn.Linear(128,128)
        self.encoding_cell = make_mlp([256,256,128], activation='relu', batch_norm=False, dropout=0)
        self.encoding_cell_out = nn.Linear(128,128)
...
#        new_hidden = (hidden[0],cell_id)
        hidden_0 = self.encoding_hidden_out(self.encoding_hidden(torch.cat((hidden[0],p_id),dim=2)))
        hidden_1 = self.encoding_cell_out(self.encoding_cell(torch.cat((hidden[1],p_id),dim=2)))
