...
if model_selection == 'Seq2Seq':
    ...
    attn = AutoEnc_RNN_Seq2Seq.Attention(ENC_HID_DIM, DEC_HID_DIM).to(device)
    enc = AutoEnc_RNN_Seq2Seq.RNN_Encoder(INPUT_DIM, ENC_EMB_DIM, ENC_HID_DIM, 'LSTM', device, N_LAYER, ENC_DROPOUT).to(device)
    dec = AutoEnc_RNN_Seq2Seq.RNN_Decoder_2(DEC_HID_DIM, OUTPUT_DIM, ENC_HID_DIM, attn, DEC_DROPOUT, 'LSTM', device, N_LAYER).to(device)
    
    au_generator = AutoEnc_RNN_Seq2Seq.Seq2Seq_Generator_1(enc, dec, device).to(device)   
...
#            for j in range(1,150):
            input_tensor = batch['AU_sequence'].to(device)
            input_len = batch['phoneme_len'].to(device)
            target_tensor = batch['AU_sequence'].to(device)
...
#            plot_loss_total_val = 0
        ...
        if epoch % save_every == 0:
            ...
            }, './checkpoints/Seq2Seq_AutoEnc_LSTM_2_epoch_%d.pth' % epoch)
