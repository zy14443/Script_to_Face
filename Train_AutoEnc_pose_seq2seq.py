...
import AutoEnc_RNN_pose_Seq2Seq
...
#file_names = sorted(glob.glob("./Data_Preprocess/train_AUs_LRW/*.mat"))
...
if model_selection == 'Seq2Seq':
    ...
    attn = AutoEnc_RNN_pose_Seq2Seq.Attention(ENC_HID_DIM, DEC_HID_DIM).to(device)
    enc = AutoEnc_RNN_pose_Seq2Seq.RNN_Encoder(INPUT_DIM, ENC_EMB_DIM, ENC_HID_DIM, 'GRU', device, N_LAYER, ENC_DROPOUT).to(device)
    dec = AutoEnc_RNN_pose_Seq2Seq.RNN_Decoder_2(DEC_HID_DIM, OUTPUT_DIM, ENC_HID_DIM, attn, DEC_DROPOUT, 'GRU', device, N_LAYER).to(device)
    
    au_generator = AutoEnc_RNN_pose_Seq2Seq.Seq2Seq_Generator_1(enc, dec, device).to(device)   
...
#            for j in range(1,150):
            input_tensor = batch['pose_sequence'].to(device)
            input_len = batch['phoneme_len'].to(device)
            ...
            output_tensor = model(input_tensor, input_len, target_tensor,tf_rate)
...
#        return epoch_loss, len(data_loader), output_tensor
    ...
    if(load_epoch):
        print('load model')
        checkpoint = torch.load('./saved_checkpoints/Seq2Seq_pose_AutoEnc_GRU_2_epoch_195.pth',map_location=device)
...
#            plot_loss_total_val = 0
        ...
        if epoch % save_every == 0:
            ...
            }, './checkpoints/Seq2Seq_pose_AutoEnc_GRU_2_epoch_%d.pth' % epoch)
